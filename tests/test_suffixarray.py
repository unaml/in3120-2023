...
class TestSuffixArray(unittest.TestCase):
    ...
    def test_canonicalized_corpus(self):
        ...
        corpus.add_document(in3120.InMemoryDocument(
            corpus.size(), {"a": "Japanese リンク"}))
        corpus.add_document(in3120.InMemoryDocument(
            corpus.size(), {"a": "Cedilla \u0043\u0327 and \u00C7 foo"}))
        engine = in3120.SuffixArray(
            corpus, ["a"], self.__normalizer, self.__tokenizer)
        # Should match "リンク".
        self.__process_query_and_verify_winner(engine, "ﾘﾝｸ", [0], 1)
        # Should match "\u0043\u0327".
        self.__process_query_and_verify_winner(engine, "\u00C7", [1], 2)
    ...
    def test_cran_corpus(self):
        ...
        self.__process_query_and_verify_winner(
            engine, "approximate solution", [159, 1374], 3)
    ...
    def test_memory_usage(self):
        ...
        corpus.add_document(in3120.InMemoryDocument(
            3, {"a": "oO" * 10000, "b": "o"}))
        corpus.add_document(in3120.InMemoryDocument(
            4, {"a": "cbab o obab O ", "b": "o o " * 10000}))
        tracemalloc.start()
        snapshot1 = tracemalloc.take_snapshot()
        engine = in3120.SuffixArray(
            corpus, ["a", "b"], self.__normalizer, self.__tokenizer)
        ...
        for statistic in snapshot2.compare_to(snapshot1, "filename"):
            if statistic.traceback[0].filename == inspect.getfile(in3120.SuffixArray):
                self.assertLessEqual(statistic.size_diff,
                                     2000000, "Memory usage seems excessive.")
    ...
    def test_multiple_fields(self):
        ...
        corpus.add_document(in3120.InMemoryDocument(
            2, {"field1": "y", "field2": "z"}))
        engine0 = in3120.SuffixArray(
            corpus, ["field1", "field2"], self.__normalizer, self.__tokenizer)
        engine1 = in3120.SuffixArray(
            corpus, ["field1"], self.__normalizer, self.__tokenizer)
        engine2 = in3120.SuffixArray(
            corpus, ["field2"], self.__normalizer, self.__tokenizer)
