...
class TestShingleGenerator(unittest.TestCase):
    ...
    def test_strings(self):
        ...
        self.assertListEqual(
            list(self.__tokenizer.strings("bana")), ["ban", "ana"])
        self.assertListEqual(list(self.__tokenizer.strings("banan")), [
                             "ban", "ana", "nan"])
        self.assertListEqual(list(self.__tokenizer.strings("banana")), [
                             "ban", "ana", "nan", "ana"])

    def test_tokens(self):
        self.assertListEqual(
            list(self.__tokenizer.tokens("ba")), [("ba", (0, 2))])
    ...
    def test_ranges(self):
        ...
        self.assertListEqual(list(self.__tokenizer.ranges("banan")), [
                             (0, 3), (1, 4), (2, 5)])
    ...
    def test_uses_yield(self):
        ...
        for i in range(0, 5):
            ...
            self.assertIsInstance(
                self.__tokenizer.ranges(text), types.GeneratorType)
            self.assertIsInstance(
                self.__tokenizer.tokens(text), types.GeneratorType)
            self.assertIsInstance(
                self.__tokenizer.strings(text), types.GeneratorType)
    ...
    def test_shingled_mesh_corpus(self):
        ...
        index = in3120.InMemoryInvertedIndex(
            corpus, ["body"], normalizer, self.__tokenizer)
